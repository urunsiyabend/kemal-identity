...
## v0.9.0 — the catalogue's second pass

**Released on 2026-09-01.** Measured first, and the scenario is named. A minor rather than a patch
because two items change behaviour an application may be observing, though no signature moved.
...
| Six worked examples, all compiled by CI | **done** — three catalogue scenarios stopped at M3 for want of one. `api_tokens`, `ownership`, `custom_bearer`, `multi_issuer_jwt` and `service_account` join `browser_session`; the CI step globs the directory so a new one cannot rot |
| `revoke` scoped to the token's owner | **done** — found while writing the API example: `DELETE /tokens/:id` written the obvious way revoked whichever token the caller named, and a token id is not secret |
...
| Evidence that the password itself was typed | **done** — `auth_sessions.password_verified_at` and `env.auth.require_recent_password!(within:)`. `authenticated_at` is restamped by every assurance increase, so a TOTP satisfied a window that meant "type your password again"; `assurance` cannot separate them either, since a federated login also reaches `Password`. One column rather than the general method→timestamp map the notes proposed: **Laravel and django-sudo both keep exactly one extra timestamp**, and the identity providers that keep the general map (Supabase, Keycloak, Auth0) ship no guard at all. `blueprints/0031` |
| A step-up refusal carrying its requirement | **done** — `StepUpRequirement` on the error, and `Authz::Forbidden#minimum_assurance` filled from the permission that refused. The old `error.max_age ? "fresh" : "mfa"` inference was already wrong once `Recovery` existed, and `require_recent_password!` made it worse by refusing with a window and meaning a different prompt. ASP.NET Core and Spring Security 7 both carry the requirement in the denial; the wire stays as it is, because `acr_values` reopens `blueprints/0028` and a Laravel-style `423` reopens `blueprints/0026`. `blueprints/0032` |

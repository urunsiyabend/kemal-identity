...
| Per-token scopes, intersected with account permissions | **not started** — the second half of 0021. `CredentialRef#scopes` and `Forbidden.out_of_scope` are in place and nothing populates them yet. Reverses the v0.4 deferral, which was correct while there was no authorizer for a scope to intersect with |

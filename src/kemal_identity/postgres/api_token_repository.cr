module KemalIdentity::Postgres
  ...
  class ApiTokenRepository < ApiTokens::Repository
    ...
    def create(token : ApiTokens::Token) : Nil
      ...
      raise InfrastructureError.new("api token already exists")
    end
    ...
    def touch(id : String, last_used_at : Time) : Bool
      result = @db.exec("UPDATE auth_api_tokens SET last_used_at = $1 WHERE id = $2", last_used_at, id)
    ...
    def revoke(id : String, at : Time) : Bool
      ...
      result = @db.exec(
        "UPDATE auth_api_tokens SET revoked_at = $1 WHERE id = $2 AND revoked_at IS NULL", at, id
      )
      result.rows_affected == 1
    end
    ...
    def revoke_all_for_account(account_id : String, at : Time) : Int32
      result = @db.exec(
        "UPDATE auth_api_tokens SET revoked_at = $1 WHERE account_id = $2 AND revoked_at IS NULL",
        at, account_id
      )
    ...
    def list_for_account(account_id : String) : Array(ApiTokens::Token)
      @db.query_all(
        "SELECT #{COLUMNS} FROM auth_api_tokens WHERE account_id = $1 ORDER BY created_at DESC, id DESC",
        account_id
      ) { |row| read(row) }
    end
    ...
    def delete_expired(before : Time) : Int32
      ...
      result = @db.exec(
        "DELETE FROM auth_api_tokens WHERE expires_at IS NOT NULL AND expires_at <= $1", before
      )
      result.rows_affected.to_i32
    end

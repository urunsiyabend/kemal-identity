module KemalIdentity::SQLite
  ...
  class AuthzRepository < Authz::Repository
    ...
    def remove_member(account_id : String, tenant_id : String) : Bool
      ...
      @db.transaction do |tx|
        ...
        result = connection.exec(
          "DELETE FROM auth_tenant_memberships WHERE account_id = ? AND tenant_id = ?",
        ...
        connection.exec(
          "DELETE FROM auth_role_assignments WHERE account_id = ? AND tenant_id = ?",
          account_id, tenant_id
        )
      end
    ...
    def member?(account_id : String, tenant_id : String) : Bool
      !@db.query_one?(
        "SELECT 1 FROM auth_tenant_memberships WHERE account_id = ? AND tenant_id = ?",
    ...
    # `IS NULL` rather than `= ?` for the global case: a global grant and a tenant grant of the
        ...
        if tenant_id.nil?
          @db.exec(<<-SQL, account_id, role)
            DELETE FROM auth_role_assignments
             WHERE account_id = ? AND role = ? AND tenant_id IS NULL
            SQL
        else
          @db.exec(<<-SQL, account_id, role, tenant_id)
            DELETE FROM auth_role_assignments
             WHERE account_id = ? AND role = ? AND tenant_id = ?
            SQL
        end
      ...
      if tenant_id.nil?
        @db.query_all(<<-SQL, role, as: String)
          SELECT account_id FROM auth_role_assignments
           WHERE role = ? AND tenant_id IS NULL
    ...
    def remove_account(account_id : String) : Int32
      ...
      @db.transaction do |tx|
        ...
        removed += connection.exec(
          "DELETE FROM auth_role_assignments WHERE account_id = ?", account_id
        ).rows_affected.to_i

        removed += connection.exec(
          "DELETE FROM auth_tenant_memberships WHERE account_id = ?", account_id
        ).rows_affected.to_i
      end
    ...
    private def global_roles(account_id : String) : Array(String)
      @db.query_all(<<-SQL, account_id, as: String)
        SELECT role FROM auth_role_assignments
         WHERE account_id = ? AND tenant_id IS NULL
         ORDER BY granted_at ASC, id ASC
        SQL
    end

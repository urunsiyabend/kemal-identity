...
describe KemalIdentity::Passwords::Authenticator do
  ...
  describe "#authenticate" do
    it "authenticates a correct password" do
      _, _, auth = build([KemalIdentity::Testing.account(password_digest: digest)], hasher)
    ...
    it "returns a principal naming the account" do
      ...
      principal.authenticated_at.should eq(KemalIdentity::Testing::FIXED_NOW)
    end
    ...
    it "is fresh, so a step-up guard immediately after login passes" do
      _, clock, auth = build([KemalIdentity::Testing.account(password_digest: digest)], hasher)
      principal = KemalIdentity::Testing.should_authenticate(
        auth.authenticate(login: "ada@example.com", password: password)
      )
    ...
    it "carries the account's tenant" do
      _, _, auth = build([
        KemalIdentity::Testing.account(tenant_id: "t1", password_digest: digest),
      ], hasher)
    ...
    it "finds an account within its tenant" do
      _, _, auth = build([
        KemalIdentity::Testing.account(id: "a1", login: "ada@example.com", password_digest: digest),
        KemalIdentity::Testing.account(id: "a2", login: "ada@example.com", tenant_id: "t1", password_digest: digest),
      ], hasher)

...
describe "logout" do
  it "rejects the session cookie afterwards" do
    ...
    KemalIdentity::SpecHelper.should_fail_with(outcome, KemalIdentity::FailureReason::Revoked)
  end
...
describe "session expiry" do
  ...
  it "rejects an expired session without waiting for the sweeper" do
    ...
    outcome = h.service.resolve(issued.token.reveal)
    KemalIdentity::SpecHelper.should_fail_with(outcome, KemalIdentity::FailureReason::Expired)
  ...
  it "is accurate to within exactly one touch_interval" do
    ...
    h.sessions.find_by_digest(issued.record.token_digest).or_fail
      .session.idle_expires_at.should eq(KemalIdentity::SpecHelper::FIXED_NOW + 30.minutes)

    # A read past it does.
    h.clock.advance(31.seconds)
    h.service.resolve(issued.token.reveal).should be_a(KemalIdentity::Authenticated)
    h.sessions.find_by_digest(issued.record.token_digest).or_fail
      .session.idle_expires_at.should eq(KemalIdentity::SpecHelper::FIXED_NOW + 61.seconds + 30.minutes)
  end
  ...
  it "fires absolute expiry regardless of activity" do
    ...
    KemalIdentity::SpecHelper.should_fail_with(outcomes[12], KemalIdentity::FailureReason::Expired)
  end
...
describe "session fixation" do
  ...
  it "makes the pre-login token unusable" do
    ...
    KemalIdentity::SpecHelper.should_fail_with(h.service.resolve(before.token.reveal), KemalIdentity::FailureReason::Revoked)
  end
  ...
  it "raises the assurance level and refreshes authenticated_at" do
    ...
    after.principal.authenticated_at.should eq(KemalIdentity::SpecHelper::FIXED_NOW + 1.hour)
  ...
  it "restarts the absolute window on re-authentication but not on activity" do
    ...
    after.record.absolute_expires_at.should eq(KemalIdentity::SpecHelper::FIXED_NOW + 1.hour + 4.hours)
  end
end
